      ...
      >
        ...
        <View className="flex-col items-start justify-center mt-5 px-5">
          ...
          <View className="flex flex-row justify-between w-1/2">
            <MovieInfo
              label="Budget"
              value={`$${Math.floor(movie?.budget / 1_000_000)} million`}
            />

            <MovieInfo
              label="Revenue"
              value={`$${Math.floor(movie?.revenue / 1_000_000)} million`}
            />      
           ...
           <MovieInfo
              ...
              value={movie?.production_companies.map((c)=> c.name).join(" - ") || "NA"}
            />
        </View>
      </ScrollView>

      <TouchableOpacity 
      className="absolute bottom-4 left-0 right-0 mx-5 bg-accent rounded-lg py-3.5
